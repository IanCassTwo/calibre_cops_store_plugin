...
def parse_book(data, base_url):
    ...
    for link in data.xpath('./*[local-name() = "link"]'):
        ...
        if rel and href and type:
            ...
            if 'http://opds-spec.org/acquisition/buy' in rel:
    ...
    if price_e:
        ...
        s.price = (currency_code + ' ' + price).strip()
    ...
    for link in data.xpath('./*[local-name() = "link"]'):
        ...
        href = link.get('href')
        type = link.get('type')
        if href and type and ('thumbnail' in rel or 'image' in rel):
            s.cover_url = link_url = urljoin(base_url, href)
            s.cover_mime = type
            break
...
def search_feed(browser, url, timeout=60):
    """Recursively parse OPDS entries"""
    with closing(browser.open(url, timeout=timeout)) as f:
        ...
        for entry in data.xpath('//*[local-name() = "entry"]'):
            if is_book(entry):
                ...
                # Follow links for nested feeds
                for link in entry.xpath('./*[local-name() = "link"]'):
                    href = link.get('href')
                    type = link.get('type')
                    if href and type:
                        next_url = urljoin(url, href)
                        yield from search_feed(browser, next_url, timeout)
...
class CopsStorePlugin(BasicStoreConfig, StorePlugin):
    ...
    def browser(self):
        ...
        if self._updateBrowser:
            if self.config['user'] and self.config['pass']:
                value = f"{self.config['user']}:{self.config['pass']}"
                auth = b64encode(value.encode('ascii')).decode('ascii')
    ...
    def search(self, query, max_results=10, timeout=60):
        ...
        base = get_template(self.browser(), self.config['url'])
        search_url = base.replace("{searchTerms}", quote_plus(query))

        count = 0
        for book in search_feed(self.browser(), search_url, timeout):
            if count >= max_results:
                return

            # Fetch cover via plugin browser and embed as data URL
            if hasattr(book, 'cover_url') and book.cover_url.startswith("http"):
                try:
                    with closing(self.browser().open(book.cover_url, timeout=30)) as f:
                        image_bytes = f.read()
                        mime = getattr(book, 'cover_mime', 'image/jpeg')
                        book.cover_url = f"data:{mime};base64," + base64.b64encode(image_bytes).decode("utf-8")
                except Exception as e:
                    print("Failed to fetch cover:", e)
